...
DEST = '/Users/pedronassiff/Desktop/proyectos/random/src/pages/AnalisisDatasets.jsx'

NEW_CONTENT = r'''import React, { useState, useCallback } from 'react'
...
function SnapshotPanel({ snap, onDismiss }) {
  ...
  const totalDelta = BANDS.reduce((sum, b) =>
    sum + Math.abs((snap.bandsDisplayRaw?.[b] ?? 0) - refRow.bands_display[b]), 0
  ) / BANDS.length
  ...
  return (
    <div className="bg-white/[0.03] border border-white/20 rounded-xl p-4 mb-6 relative">
        ...
        <thead>
          <tr className="border-b border-white/10">
            ...
            <th className="text-left py-1 px-1 text-[9px] text-white/40 uppercase">
              Live (EMA)<br/><span className="text-white/20 normal-case font-normal">bands_display</span>
            </th>
            <th className="text-left py-1 px-1 text-[9px] text-white/40 uppercase">
              Raw (pre-EMA)<br/><span className="text-white/20 normal-case font-normal">bands_display_raw · Δ vs EMA</span>
            </th>
            <th className="text-left py-1 px-1 text-[9px] text-white/40 uppercase">
              Referencia t={refRow.t}s<br/><span className="text-white/20 normal-case font-normal">reference_data.json · Δ raw vs ref</span>
            </th>
          </tr>
        </thead>
        <tbody>
      ...
      <div className="grid grid-cols-3 gap-2 text-[10px] pt-3 border-t border-white/[0.06]">
        <div><span className="text-white/30">PLV live: </span><span className="font-mono text-white/70">{(snap.plv ?? snap.coherence ?? 0).toFixed(3)}</span></div>
        <div>
          <span className="text-white/30">PLV ref: </span>
          <span className="font-mono text-white/70">{refRow.coherence.toFixed(3)}</span>
          <Delta a={snap.plv ?? snap.coherence ?? 0} b={refRow.coherence} />
        </div>
      ...
      <div className="mt-3 pt-3 border-t border-white/[0.06] text-[10px] space-y-0.5">
        ...
        {BANDS.map(b => {
          const rawVal = snap.bandsDisplayRaw?.[b]
          const refVal = refRow.bands_display[b]
          if (rawVal == null) return null
          const diff = rawVal - refVal
          const abs  = Math.abs(diff)
          if (abs < 0.03) return null
          return (
            <p key={b}>
              <span style={{ color: BAR_COLORS[b] }}>{b}</span>
              {': raw '}<span className="text-white/50">{(rawVal*100).toFixed(1)}%</span>
        ...
        {snap.state !== refRow.state && snap.stateRaw !== refRow.state && (
          <p className="text-orange-400">Estado live ({snap.stateRaw}) ≠ ref ({refRow.state})</p>
        )}
        {totalDelta < 0.02 && (
                  ...
                  <tr className={`border-b border-white/10 ${sc.bg}`}>
                    ...
                    {BANDS.map(b => (
                      <td key={b} className="py-1 px-1">
                        <div className="flex items-center">
                          <Bar value={snap.bandsRaw?.[b] ?? 0} color={BAR_COLORS[b]} />
                          <Delta a={snap.bandsRaw?.[b]} b={row.bands[b]} />
                        </div>
                      </td>
                    ))}
                    <td className="text-white/10 px-1">│</td>
                    ...
                    {BANDS.map(b => (
                      <td key={b} className="py-1 px-1">
                        <div className="flex items-center">
                          <Bar value={snap.bandsDisplayRaw?.[b] ?? 0} color={BAR_COLORS[b]} maxVal={0.65} />
                          <Delta a={snap.bandsDisplayRaw?.[b]} b={row.bands_display[b]} />
                        </div>
                      </td>
                    ))}
                    <td className="text-white/10 px-1">│</td>
...
function CaptureButton({ onCapture }) {
  const { sessionProgress, sessionTimestamp, bands, bandsDisplay,
          bandsRaw, bandsDisplayRaw, state, stateRaw, plv, coherence } = useBrainStore()
    ...
    <div className="min-h-screen bg-black text-white px-4 py-8 max-w-screen-2xl mx-auto">
      ...
      {!sessionProgress && (
        <div className="mb-6 px-4 py-3 bg-amber-900/20 border border-amber-500/20 rounded-xl text-xs text-amber-200/60">
          <strong>Sin sesión activa.</strong> Ve a <code>/lab/brain</code>, inicia la sesión de meditación,
          pausa en el segundo que quieras analizar, y pulsa "Capturar ahora".
        </div>
      )}
...
lines = len(open(DEST).readlines())
print(f"Written {lines} lines to {DEST}")
